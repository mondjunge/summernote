...
describe('paste-from-word: empty block removal', () => {
  ...
  it('preserves empty paragraphs as <p><br></p> spacers', () => {
    // Word uses empty paragraphs for visual spacing — they should be kept
  ...
  it('removes empty divs and spans but keeps empty paragraphs', () => {
    const result = cleaner.clean('<div></div><p></p><span></span>');
    expect(result).not.toContain('<div>');
    expect(result).not.toContain('<span>');
    expect(result).toContain('<p><br></p>');
  });
...
describe('paste-from-word: table border normalization', () => {
  let cleaner;
  beforeEach(() => { cleaner = new WordCleaner(); });

  it('collapses Word Online verbose border longhands to shorthand', () => {
    const input = '<table><tbody><tr>' +
      '<td style="border-width: 1px; border-style: solid; ' +
      'border-color: rgb(170, 170, 170) rgb(0, 0, 0) rgb(0, 0, 0) rgb(170, 170, 170); ' +
      'border-image: initial;">cell</td>' +
      '</tr></tbody></table>';
    const result = cleaner.clean(input);
    expect(result).toContain('border: 1px solid rgb(170, 170, 170)');
    expect(result).not.toContain('border-width');
    expect(result).not.toContain('border-image');
  });

  it('normalizes Excel .5pt border to 1px', () => {
    const input = '<table><tbody><tr>' +
      '<td style="border: .5pt solid black;">cell</td>' +
      '</tr></tbody></table>';
    const result = cleaner.clean(input);
    expect(result).toContain('border: 1px solid black');
    expect(result).not.toContain('.5pt');
  });

  it('strips border: none from table cells', () => {
    const input = '<table><tbody><tr>' +
      '<td style="border: none;">cell</td>' +
      '</tr></tbody></table>';
    const result = cleaner.clean(input);
  ...
  it('strips font-size from table element', () => {
    const input = '<table style="font-size: 12px;"><tbody><tr>' +
      '<td>cell</td></tr></tbody></table>';
    const result = cleaner.clean(input);
